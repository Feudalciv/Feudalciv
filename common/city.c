...
int city_turns_to_grow(const struct city *pcity)
{
  if (pcity->surplus[O_FOOD] > 0) {
    ...
    return -1 + -(int)ceil(log((double)city_population_for_size(city_size_get(pcity) - 1.0) /
            city_population(pcity))/log(1.0 + game.info.pop_base_growth + game.info.pop_base_growth * pcity->surplus[O_FOOD]));
  } else {
    return FC_INFINITY;
  }
}
...
/****************************************************************************
  Calculate output (gold, science, and luxury) generated by the buildings
  of a city.  The output[] array is not cleared but is just added to.
****************************************************************************/
void add_building_output(const struct city *pcity, int *output)
{
  int * bonus;
    bonus = get_building_output_bonus(pcity);

    output_type_iterate(stat) {
      output[stat] += bonus[stat];
    } output_type_iterate_end;
    FC_FREE(bonus);
}
...
inline void set_city_production(struct city *pcity)
{
  ...
  output_type_iterate(o) {
    pcity->prod[o] = pcity->citizen_base[o] + pcity->building_base[o];
  } output_type_iterate_end;
...
void city_refresh_from_main_map(struct city *pcity, bool *workers_map)
{
  ...
  output_type_iterate(stat) {
    pcity->building_base[stat] = 0;
  } output_type_iterate_end;
  add_building_output(pcity, pcity->building_base);
